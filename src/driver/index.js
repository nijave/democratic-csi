...
// Success is verified by a follow-up state query, not the exit code (iscsiadm
// errors on an already-absent session/record).
...
// True when iscsiadm cannot run on this host at all (open-iscsi not installed,
// or the wrapper cannot reach the host binary/daemon), as opposed to an
// operational failure of a working iscsiadm (timeout, lock contention, etc).
...
// options.iscsiExpected (default true): when false, an unusable iscsiadm
// (NFS/SMB-only node without open-iscsi) means "nothing to reclaim" rather
// than a failed RPC.
...
class CsiBaseDriver {
   ...
   * node.iscsi.nodeDbSweeper.enabled). Called from node service startup.
   */
  ...
  async NodeUnstageVolume(call) {
    ...
    switch (driver.__getNodeOsDriver()) {
      case NODE_OS_DRIVER_POSIX:
        ...
        // only force-clear a confirmed-dead target; never a healthy busy mount
        ...
        if (result) {
          try {
            // small "busy" retry only; the CO drives real retries so we don't hold the lock
            ...
            if (err.timeout) {
              ...
              switch (result.fstype) {
                ...
                case "nfs4":
                  ...
                  break;
                default:
                  // dead iscsi target -> reclaim session, otherwise rethrow (retryable)
                  await handleUmountFailure(err);
                  break;
              }
            } else {
              // non-timeout umount failure (e.g. EIO on offline target): reclaim if dead, else rethrow
              await handleUmountFailure(err);
            }
          }
        }

        // iSCSI is "expected" only with evidence (dead target or an iscsi-attached
        // device); otherwise a node without open-iscsi must not fail the RPC.
              ...
              case "nvme":
                {
                  if (
                    ...
                    if (nqn) {
                      // disconnect handles all devices for the nqn, so stop iterating DM multipath slaves
